import type { ComponentPropsWithRef } from "react";

import { IconBase } from "../shared/icon";

export interface UserIconProps extends ComponentPropsWithRef<typeof IconBase> {}

export function UserIcon({ ref, className, ...props }: UserIconProps) {
	return (
		<IconBase
			{...props}
			ref={ref}
			className={className}
		>
			<g>
				<g>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M12 0.5C9.37665 0.5 7.25 2.62665 7.25 5.25C7.25 7.87335 9.37665 10 12 10C14.6234 10 16.75 7.87335 16.75 5.25C16.75 2.62665 14.6234 0.5 12 0.5ZM9.25 5.25C9.25 3.73122 10.4812 2.5 12 2.5C13.5188 2.5 14.75 3.73122 14.75 5.25C14.75 6.76878 13.5188 8 12 8C10.4812 8 9.25 6.76878 9.25 5.25Z"
						fill="currentColor"
					/>
					<path
						fillRule="evenodd"
						clipRule="evenodd"
						d="M12 12C4.13498 12 1.5 16.2774 1.5 18.5C1.5 19.4772 1.83011 20.3693 2.48039 21.0196C3.13067 21.6699 4.02275 22 5 22H19C19.9772 22 20.8693 21.6699 21.5196 21.0196C22.1699 20.3693 22.5 19.4772 22.5 18.5C22.5 16.2774 19.865 12 12 12ZM3.5 18.5C3.5 17.7226 4.86502 14 12 14C19.135 14 20.5 17.7226 20.5 18.5C20.5 19.0228 20.3301 19.3807 20.1054 19.6054C19.8807 19.8301 19.5228 20 19 20H5C4.47725 20 4.11933 19.8301 3.89461 19.6054C3.66989 19.3807 3.5 19.0228 3.5 18.5Z"
						fill="currentColor"
					/>
				</g>
			</g>
		</IconBase>
	);
}
