...
/**
 * Creates generated audience wallets for one label.
 */
function getLabelsWallets(id: number): LabelWallet[] | undefined {
	if (id > GENERATED_ENTITY_COUNT) return undefined;

	return Array.from({ length: 50 }, (_, index): LabelWallet => {
...
export async function GET(request: Request, context: RouteParams<"labelID">): Promise<Response> {
	...
	let wallets = getLabelsWallets(id);
