...
/**
 * Creates generated audience wallets for one dapp.
 */
function getDappsWallets(id: number): DappWallet[] | undefined {
	if (id > GENERATED_ENTITY_COUNT) return undefined;

	return Array.from({ length: 50 }, (_, index): DappWallet => {
...
export async function GET(request: Request, context: RouteParams<"dappID">): Promise<Response> {
	...
	let wallets = getDappsWallets(id);
