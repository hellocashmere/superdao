...
import type { AudienceOverlapFixture, ChartDatumFixture, InfluencerFixture, InsightsFixture } from "../types/insights";
...
const names = [DEMO_PROFILE.title, "vitalik.eth", "punk6529.eth", "cryptoboss.eth", "daoist.eth"];
...
/**
 * Converts deterministic chart seeds into serialized chart data.
 */
function createChart(seeds: ReadonlyArray<readonly [string, number, string?]>): ChartDatumFixture[] {
	return seeds.map(([label, value]) => ({ label, value }));
}

/**
 * Creates the shared deterministic insights payload for a valid resource ID.
 */
export function createInsights(id: number, resourceCount: number): InsightsFixture | undefined {
	if (id > resourceCount) return undefined;

	const influencers = Array.from(
		{ length: 5 },
		(_, index): InfluencerFixture => ({
			title: names[index] ?? "Wallet",
			...
			...(index === 0 ? DEMO_PROFILE.influencer : {}),
		})
	);
	const overlap = Array.from(
		{ length: 25 },
		(_, index): AudienceOverlapFixture => ({
			title: collections[(id + index) % collections.length] ?? "Collection",
	...
	return {
		...
		wallet_balance: balanceDistributionSeeds.map(([label, value]) => ({ label, value })),
		nft_allocation: createChart([
			["1-19", 10_000, "10k"],
			["20-39", 7_000, "7k"],
			["40-59", 6_000, "6k"],
			["60-79", 3_000, "3k"],
		...
		transaction_stats: [
			{ label: "Count", value: 258_940, variant: "default" },
			{ label: "Volume", value: 370_827.38, variant: "default" },
			{ label: "Income", value: 403_735.5, variant: "positive" },
			{ label: "Outcome", value: -429_040.02, variant: "negative" },
		],
		contact_metrics: [
			{
				title: "Twitter",
		...
		interests: createChart([
			["Luxury", 97_500, "97.5k"],
			["Art", 43_200, "43.2k"],
			["Music", 12_700, "12.7k"],
			["Fashion", 0, "0"],
		]),
		personas: createChart([
			["Voter", 37_200, "37.2k"],
			["Gamer", 31_500, "31.5k"],
