 ...
 */
export function SidebarSearchTrigger({ className, onActivate, ref, ...props }: SidebarSearchTriggerProps) {
  return (
    <div
      ref={ref}
 ...
 */
export function SidebarSearchAvatar({ className, ref, result, ...props }: SidebarSearchAvatarProps) {
  let glyph = null;
...
export function SidebarSearchPanel({
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <div
          data-slot="sidebar-search-loading"
          className="absolute inset-x-5 top-1/2 flex -translate-y-1/2 items-center justify-center"
        >
          <Spinner className="size-5 text-[#a2a8b4]" />
        </div>
          ...
          <div className="flex flex-col gap-0.5">
            {visibleResults.map((result) => (
              <Link
                key={result.id}
                href={result.href}
                className="flex h-[46px] w-full items-center gap-3 px-5 py-1 outline-none hover:bg-[#d0dcf50a] focus-visible:bg-[#d0dcf50a]"
                onClick={() => onResultSelect(result.id)}
              >
                <SidebarSearchAvatar result={result} />
                <span className="flex min-w-0 flex-col items-start">
                  <span className="max-w-full truncate text-sm/5 font-semibold text-white">{result.name}</span>
