...
import type { EmailSettingsFormValues } from "../model/form";
import { emailSettingsFormSchema, getEmailSettingsFormDefaults, normalizeEmailSettingsForm } from "../model/form";
 ...
 */
export function EmailSettingsForm({ ref, className, ...props }: EmailSettingsFormProps) {
	const userID = useSessionStore((state) => state.userID);
	const user = useUserStore((state) => state.users.find((candidate) => candidate.id === userID));
	const updateUser = useUserStore((state) => state.updateUser);
	const [isPending, setIsPending] = useState<boolean>(false);
	const [pendingEmail, setPendingEmail] = useState<string | null>(null);
	const [secondsUntilResend, setSecondsUntilResend] = useState<number>(0);
	const form = useForm<EmailSettingsFormValues>({
		resolver: zodResolver(emailSettingsFormSchema),
		defaultValues: getEmailSettingsFormDefaults(user),
	});
	...
	/**
	 * Normalizes the email and starts pending verification.
	 */
	function saveEmail(values: EmailSettingsFormValues) {
		if (!user) return;

		const normalized = normalizeEmailSettingsForm(values);
	...
	/**
	 * Restarts the email verification cooldown.
	 */
	...
	/**
	 * Removes the current email and clears verification state.
	 */
