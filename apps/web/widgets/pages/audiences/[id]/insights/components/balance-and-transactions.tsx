...
import { AudienceTransactionStats } from "./transaction-stats";

const walletBalanceChartConfig = {
	value: {
		label: "Wallets",
		color: "var(--chart-1)",
	},
...
const nftAllocationChartConfig = {
	value: {
		label: "Wallets",
		color: "var(--chart-2)",
	},
...
export interface AudienceBalanceAndTransactionsProps extends ComponentPropsWithRef<"section"> {
	/**
 ...
 */
export function AudienceBalanceAndTransactions({
	ref,
	className,
	metrics,
	nftAllocation,
	...
	walletBalance,
	...props
}: AudienceBalanceAndTransactionsProps) {
	const { compact, currency, number, percentValue } = useNumberFormatter();

	return (
		<section
			ref={ref}
			data-slot="audience-balance-and-transactions"
			className={className}
			{...props}
		>
