...
export interface AudienceOverlapProps extends ComponentPropsWithRef<"section"> {
	overlap: readonly LabelOverlapView[];
}

/**
 * Composes the audience overlap section.
 */
export function AudienceOverlap({ ref, className, overlap, ...props }: AudienceOverlapProps) {
	const { compact, number, percentValue } = useNumberFormatter();

	return (
		<section
			ref={ref}
			data-slot="audience-audience-overlap"
			className={className}
			{...props}
		>
			...
			<Card>
				<div className="pb-2.5">
					<Table className="min-w-250 [&_tbody_tr]:h-14 [&_td]:h-14 [&_td]:px-5 [&_td]:py-0 [&_td]:text-sm/5 [&_td]:text-[#a2a8b4] [&_td:first-child]:w-[49px] [&_th]:h-[54px] [&_th]:px-5 [&_th]:pt-6 [&_th]:pb-3 [&_th]:text-[13px]/[18px] [&_th]:font-semibold [&_th]:text-[#717a8c] [&_th:first-child]:w-[49px] [&_thead_tr]:hover:bg-transparent">
						<TableHeader>
							<TableRow>
								<TableHead>#</TableHead>
