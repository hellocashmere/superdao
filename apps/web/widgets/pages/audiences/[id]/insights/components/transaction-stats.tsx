...
export interface AudienceTransactionStatsProps extends ComponentPropsWithRef<typeof Card> {
 ...
 */
export function AudienceTransactionStats({ ref, className, stats, ...props }: AudienceTransactionStatsProps) {
	const { compact, currency } = useNumberFormatter();

	return (
		<Card
			ref={ref}
			data-slot="audience-transaction-stats"
			className={className}
			{...props}
		>
