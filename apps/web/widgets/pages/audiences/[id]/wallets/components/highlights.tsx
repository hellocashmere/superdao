...
export interface AudienceHighlightsProps extends ComponentPropsWithRef<"section"> {
	sourceLabelID: number;
}

/**
 * Renders audience highlights for the selected audience.
 */
export function AudienceHighlights({ ref, className, sourceLabelID, ...props }: AudienceHighlightsProps) {
	const { compact, currency, number, percentValue } = useNumberFormatter();
	const highlightsQuery = useGetLabelHighlights(sourceLabelID);
	...
	return (
		...
		>
			...
			<BarChartCard
				...
				tooltip="Distribution of audience wallets by their combined token balance in USD across Ethereum and Polygon."
