...
import type { ReportingConversionDatumView } from "@/entities/reporting";

const chartConfig = {
	pageView: {
		label: "PAGE_VIEW › WALLET_CONNECT",
		color: "#398fe5",
	},
	walletConnect: {
 ...
 */
export function ConversionChart({ ref, className, data, ...props }: ConversionChartProps) {
	return (
		<Card
			ref={ref}
		...
		>
			...
			<ChartContainer
				...
				className="aspect-auto h-57.5 w-full px-2 pb-2"
			>
				<LineChart
					data={data}
				...
				{payload.map((item) => {
					...
					return (
						...
						>
							...
							>
								{item.value === undefined ? "—" : percentValue(item.value)}
							</span>
