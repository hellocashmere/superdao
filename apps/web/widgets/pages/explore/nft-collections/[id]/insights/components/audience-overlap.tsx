...
import type { NftCollectionOverlapView } from "@/entities/nft-collection";

export interface CollectionAudienceOverlapProps extends ComponentPropsWithRef<"section"> {
	overlap: readonly NftCollectionOverlapView[];
}

/**
 * Composes the collection audience overlap section.
 */
export function CollectionAudienceOverlap({ ref, className, overlap, ...props }: CollectionAudienceOverlapProps) {
	const { compact, number, percentValue } = useNumberFormatter();

	return (
		<section
			ref={ref}
			data-slot="collection-audience-overlap"
			className={className}
			{...props}
		>
