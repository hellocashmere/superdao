...
import type { NftCollectionTransactionStatView } from "@/entities/nft-collection";

export interface CollectionTransactionStatsProps extends ComponentPropsWithRef<typeof Card> {
	stats: readonly NftCollectionTransactionStatView[];
}

/**
 * Renders aggregate transaction statistics for the last 30 days.
 */
export function CollectionTransactionStats({ ref, className, stats, ...props }: CollectionTransactionStatsProps) {
	const { compact, currency } = useNumberFormatter();

	return (
		<Card
			ref={ref}
			data-slot="collection-transaction-stats"
			className={className}
			{...props}
		>
