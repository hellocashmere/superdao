...
import { useGetNftCollectionWallets } from "@/entities/nft-collection";

import { CollectionWalletsExportDialog } from "../export-dialog";

import { columnLabels, columns, dataTableFeatures, filterLabels } from "./table-config";

export interface CollectionWalletsTableProps extends ComponentPropsWithRef<typeof Card> {
	/**
	 * Backend-provided ID of the NFT collection whose wallets populate the table.
	 */
	collectionID: number;
}

/**
 * Renders the searchable, sortable, and paginated collection wallet table.
 */
export function CollectionWalletsTable({ ref, className, collectionID, ...props }: CollectionWalletsTableProps) {
	const walletsQuery = useGetNftCollectionWallets(collectionID);
