...
export interface CollectionWalletRankProps extends ComponentPropsWithRef<"span"> {
  rank: string;
  tone: NftCollectionWalletView["rankTone"];
}

/**
 * Renders the color-coded Superrank value for a wallet.
 */
export function CollectionWalletRank({ className, rank, ref, tone, ...props }: CollectionWalletRankProps) {
  return (
    <span
      {...props}
      ref={ref}
      data-slot="nft-collection-wallet-rank"
      data-tone={tone}
      className={cn(
