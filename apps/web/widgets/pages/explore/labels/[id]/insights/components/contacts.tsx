...
export interface LabelContactsInsightsProps extends ComponentPropsWithRef<"section"> {
 ...
 */
export function LabelContactsInsights({ ref, className, influencers, metrics, ...props }: LabelContactsInsightsProps) {
	const { compact, currency, number, percentValue } = useNumberFormatter();

	return (
		<section
			ref={ref}
			data-slot="label-contacts-insights"
			className={className}
			{...props}
		>
