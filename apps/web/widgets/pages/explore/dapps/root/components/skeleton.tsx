...
export interface DappsRootSkeletonProps extends ComponentPropsWithRef<typeof Container> {}

const skeletonRows = Array.from({ length: 12 }, (_, index) => index);

/**
 * Renders the loading layout for the dapps root page.
 */
export function DappsRootSkeleton({ className, ref, ...props }: DappsRootSkeletonProps) {
  return (
    <Container
      {...props}
      ref={ref}
      aria-busy="true"
      data-slot="dapps-root-skeleton"
