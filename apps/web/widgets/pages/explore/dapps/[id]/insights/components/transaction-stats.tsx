...
import type { DappTransactionStatView } from "@/entities/dapp";

export interface DappTransactionStatsProps extends ComponentPropsWithRef<typeof Card> {
	stats: readonly DappTransactionStatView[];
}

/**
 * Renders aggregate transaction statistics for the last 30 days.
 */
export function DappTransactionStats({ ref, className, stats, ...props }: DappTransactionStatsProps) {
	const { compact, currency } = useNumberFormatter();

	return (
		<Card
			ref={ref}
			data-slot="dapp-transaction-stats"
			className={className}
			{...props}
		>
				...
				<div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
					{stats.map((stat) => (
						<div key={stat.label}>
							...
							>
								{stat.label === "Count" ? compact(stat.value) : currency(stat.value, "USD")}
							</p>
