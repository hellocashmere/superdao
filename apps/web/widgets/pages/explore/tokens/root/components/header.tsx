...
export interface TokensRootHeaderProps extends ComponentPropsWithRef<typeof PageHeader> {
  onClearSearch: () => void;
...
/**
 * Renders the tokens directory heading, count, and search field.
 */
export function TokensRootHeader({
  className,
  onClearSearch,
  onSearchChange,
  ref,
  search,
  ...props
}: TokensRootHeaderProps) {
  return (
    <PageHeader
      {...props}
      ref={ref}
      data-slot="tokens-root-header"
      className={cn("flex min-h-18 items-center justify-between gap-5", className)}
    >
      <div className="flex items-end gap-3">
        <h1 className="text-2xl/7 font-bold">Tokens</h1>
