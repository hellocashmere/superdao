...
import type { TokenTransactionStatView } from "@/entities/token";

export interface TokenTransactionStatsProps extends ComponentPropsWithRef<typeof Card> {
	stats: readonly TokenTransactionStatView[];
}

/**
 * Renders aggregate transaction statistics for the last 30 days.
 */
export function TokenTransactionStats({ ref, className, stats, ...props }: TokenTransactionStatsProps) {
	const { compact, currency } = useNumberFormatter();

	return (
		<Card
			ref={ref}
			data-slot="token-transaction-stats"
			className={className}
			{...props}
		>
