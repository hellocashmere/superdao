...
import type { TokenOverlapView } from "@/entities/token";

export interface TokenAudienceOverlapProps extends ComponentPropsWithRef<"section"> {
	overlap: readonly TokenOverlapView[];
}

/**
 * Composes the token audience overlap section.
 */
export function TokenAudienceOverlap({ ref, className, overlap, ...props }: TokenAudienceOverlapProps) {
	const { compact, number, percentValue } = useNumberFormatter();

	return (
		<section
			ref={ref}
			data-slot="token-audience-overlap"
			className={className}
			{...props}
		>
