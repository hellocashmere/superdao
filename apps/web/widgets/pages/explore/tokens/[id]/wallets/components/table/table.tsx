...
import { TokenWalletsExportDialog } from "../export-dialog";

import { columnLabels, columns, dataTableFeatures, filterLabels } from "./table-config";

export interface TokenWalletsTableProps extends ComponentPropsWithRef<typeof Card> {
	/**
	 * Backend-provided ID of the token whose wallets populate the table.
	 */
	tokenID: number;
}

/**
 * Renders the searchable, sortable, and paginated token wallet table.
 */
export function TokenWalletsTable({ ref, className, tokenID, ...props }: TokenWalletsTableProps) {
