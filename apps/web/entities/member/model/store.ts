...
import type { CommunityMember, MemberState, MemberStore } from "./types/types";
...
const featuredMembers: readonly CommunityMember[] = [
	{
		...
		avatar: getMemberAvatarUrl("/avatars/cashmere.png"),
		role: "Owner",
		assignedBy: null,
	...
	},
	{
		...
		avatar: getMemberAvatarUrl("/avatars/43c942bced82918ce823e09d161b77af.png"),
		role: "Admin",
		assignedBy: "Cashmere R.",
		joinDate: "Jun 12, 2023",
		organizationID: "acme",
	},
	{
		...
		avatar: getMemberAvatarUrl("/avatars/13f789cec096eaa9226cb1759fc74954.png"),
		role: "Member",
		assignedBy: "Cashmere R.",
	...
	},
	{
		...
		avatar: getMemberAvatarUrl("/avatars/190842423a18e1e6e14e3cc9e06bf656.png"),
		role: "Member",
...
function getMemberAvatar(index: number) {
	return getMemberAvatarUrl(memberAvatars[(index - 1) % memberAvatars.length]!);
}

/**
 * Resolves an API-hosted member avatar path.
 */
function getMemberAvatarUrl(path: string): string {
...
const additionalMembers: readonly CommunityMember[] = additionalMemberNames.map((name, position) => {
	...
	return {
		...
		assignedBy: position % 3 === 0 ? "Aster Quinn" : "Cashmere R.",
...
/**
 * Reassigns sequential identifiers after the member list changes.
 */
					...
					return {
						...
						members: state.members.map((member) => {
							...
							return {
								...
								assignedBy:
									member.assignedBy === "frontman.eth" ||
									member.assignedBy === "cashmere.ton" ||
									member.assignedBy === "cashmere.gram" ||
									member.assignedBy === "Cashmere R"
										? "Cashmere R."
										: member.assignedBy,
								name:
									member.id === "cashmere-author" ||
									member.name === "frontman.eth" ||
									member.name === "cashmere.ton" ||
									member.name === "cashmere.gram" ||
									member.name === "bear.eth" ||
									member.name === "nftbear.eth" ||
									member.name === "Cashmere R"
										? "Cashmere R."
										: member.name,
							};
						}),
